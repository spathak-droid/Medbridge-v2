...
export function ClinicianSignup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await signUp(email, password, 'clinician', name)
      navigate('/dashboard')
    } catch (err: unknown) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-neutral-900">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">Create Clinician Account</h1>
          <p className="text-neutral-400 mt-2">Manage your patients with AI coaching</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-neutral-800 border border-neutral-700 rounded-2xl p-6 shadow-xl space-y-4">
          ...
          <div className="text-center text-sm text-neutral-400">
            ...
            <Link to="/clinician/login" className="text-primary-400 font-medium hover:underline">
              Sign in
            </Link>
          </div>
        </form>
